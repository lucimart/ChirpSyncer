...
class TestCleanupExpiredSessions:
    ...
    def test_cleanup_expired_sessions_deletes_expired(self, setup_db):
        ...
        conn = sqlite3.connect(setup_db)
        cursor = conn.cursor()
        cursor.execute('SELECT COUNT(*) FROM user_sessions WHERE session_token = ?', ('expired_token',))
    ...
    def test_cleanup_expired_sessions_keeps_valid(self, setup_db):
        ...
        conn = sqlite3.connect(setup_db)
        cursor = conn.cursor()
        cursor.execute('SELECT COUNT(*) FROM user_sessions WHERE session_token = ?', ('valid_token',))
        count = cursor.fetchone()[0]
        conn.close()
...
class TestBackupDatabase:
    ...
    def test_backup_database_creates_directory(self, setup_db):
        ...
        backup_dir = os.path.join(tempfile.gettempdir(), 'test_backups_' + str(int(time.time())))
...
class TestCleanupInactiveCredentials:
    ...
    def test_cleanup_inactive_credentials_marks_old(self, setup_db):
        ...
        cred_manager.save_credentials(user_id, 'twitter', 'scraping', {'username': 'test'})

        # Set last_used to 7 months ago
        old_time = int(time.time()) - (7 * 30 * 24 * 3600)
        conn = sqlite3.connect(setup_db)
        cursor = conn.cursor()
        cursor.execute('UPDATE user_credentials SET last_used = ? WHERE user_id = ?', (old_time, user_id))
        conn.commit()
        conn.close()
        ...
        conn = sqlite3.connect(setup_db)
        cursor = conn.cursor()
        cursor.execute('SELECT is_active FROM user_credentials WHERE user_id = ?', (user_id,))
    ...
    def test_cleanup_inactive_credentials_keeps_recent(self, setup_db):
        ...
        conn = sqlite3.connect(setup_db)
        cursor = conn.cursor()
        cursor.execute('UPDATE user_credentials SET last_used = ? WHERE user_id = ?', (recent_time, user_id))
        conn.commit()
        conn.close()
...
class TestCleanupErrorLogs:
    ...
    def test_cleanup_error_logs_deletes_old_errors(self, setup_db):
        ...
        old_time = int(time.time()) - (40 * 24 * 3600)
        log_audit(1, 'test_error', success=False, details={'error': 'test'}, db_path=setup_db)

        # Update created_at to be old
        conn = sqlite3.connect(setup_db)
        cursor = conn.cursor()
        cursor.execute('UPDATE audit_log SET created_at = ? WHERE success = 0', (old_time,))
        conn.commit()
        conn.close()
    ...
    def test_cleanup_error_logs_keeps_recent_errors(self, setup_db):
        ...
        conn = sqlite3.connect(setup_db)
        cursor = conn.cursor()
        cursor.execute('UPDATE audit_log SET created_at = ? WHERE success = 0', (recent_time,))
        conn.commit()
        conn.close()
    ...
    def test_cleanup_error_logs_keeps_success_logs(self, setup_db):
        ...
        old_time = int(time.time()) - (40 * 24 * 3600)
        log_audit(1, 'test_success', success=True, details={'data': 'test'}, db_path=setup_db)

        conn = sqlite3.connect(setup_db)
        cursor = conn.cursor()
        cursor.execute('UPDATE audit_log SET created_at = ? WHERE success = 1', (old_time,))
        conn.commit()
        conn.close()
...
class TestErrorHandling:
    ...
    def test_backup_database_invalid_source(self):
        ...
        try:
            result = backup_database(db_path='/nonexistent/database.db', backup_dir=backup_dir)
