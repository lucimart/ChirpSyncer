...
def regular_user(user_manager):
    ...
    user_id = user_manager.create_user(
        "testuser", "test@example.com", "Test123!@#", is_admin=False
    )
...
def admin_user(user_manager):
    ...
    user_id = user_manager.create_user(
        "admin", "admin@example.com", "Admin123!@#", is_admin=True
    )
    return user_manager.get_user_by_id(user_id)
...
def test_require_self_or_admin_blocks_other_user(client, regular_user, user_manager):
    ...
    other_user_id = user_manager.create_user(
        "other", "other@example.com", "Other123!@#", is_admin=False
    )
...
def test_require_self_or_admin_allows_admin_any_profile(
