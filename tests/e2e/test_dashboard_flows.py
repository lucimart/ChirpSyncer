...
class TestUserRegistrationToFirstSync:
    ...
    def test_user_registration_to_first_sync(self, client, user_manager, credential_manager):
        """
        ...
        add_cred_response = client.post('/credentials/add', data={
            'platform': 'twitter',
        ...
        assert b'twitter' in creds_list_response.data.lower() or b'TWITTER' in creds_list_response.data
        ...
        assert creds_metadata[0]['is_active'] == 1  # SQLite returns integers for booleans
...
class TestDashboardAnalyticsWorkflow:
    ...
    def test_dashboard_analytics_workflow(self, client, user_manager, analytics_tracker):
        """
        ...
        top_tweets = analytics_tracker.get_top_tweets(user_id, metric='engagement_rate', limit=2)
        ...
        top_tweets_response = client.get('/api/analytics/top-tweets?limit=10&metric=engagement_rate')
...
class TestCredentialManagementWorkflow:
    ...
    def test_credential_management_workflow(self, client, user_manager, credential_manager):
        """
        ...
        twitter_response = client.post('/credentials/add', data={
            'platform': 'twitter',
            'credential_type': 'api',
        ...
        bluesky_response = client.post('/credentials/add', data={
            'platform': 'bluesky',
            'credential_type': 'api',
        ...
        twitter_cred_id = next(c['id'] for c in creds_list if c['platform'] == 'twitter')
        bluesky_cred_id = next(c['id'] for c in creds_list if c['platform'] == 'bluesky')

        # Step 5: Edit Twitter API credentials
        edit_response = client.post(f'/credentials/{twitter_cred_id}/edit', data={
        ...
        delete_response = client.post(
            f'/credentials/{twitter_cred_id}/delete',
            follow_redirects=True
        )
        ...
        delete_bluesky_response = client.post(
            f'/credentials/{bluesky_cred_id}/delete',
            follow_redirects=True
        )
...
class TestAdminUserManagement:
    ...
    def test_admin_user_management(self, client, user_manager, credential_manager):
        ...
        admin_twitter_response = client.post('/credentials/add', data={
            'platform': 'twitter',
            'credential_type': 'scraping',
        ...
        share_response = client.post('/credentials/share', data={
            'credential_id': cred_id,
            'user_ids': str(new_user_id)
        }, follow_redirects=True)
        ...
        update_response = client.post(f'/users/{new_user_id}/edit', data={
        ...
        promote_response = client.post(f'/users/{new_user_id}/edit', data={
            'email': 'newemail@example.com',
            'is_active': '1',
            'is_admin': '1'
        }, follow_redirects=True)
        ...
        deactivate_response = client.post(f'/users/{new_user_id}/edit', data={
            'email': 'newemail@example.com',
            'is_active': '0',
            'is_admin': '1'
        }, follow_redirects=True)
        ...
        delete_response = client.post(
            f'/users/{regular_user_id}/delete',
            follow_redirects=True
        )
        assert delete_response.status_code == 200
...
class TestFormValidation:
    ...
    def test_registration_form_validation(self, client, user_manager):
        ...
        # Test missing fields
        missing_username = client.post('/register', data={
        ...
        password_mismatch = client.post('/register', data={
            'username': 'testuser',
            'email': 'test@example.com',
            'password': 'Test123!@#',
        ...
        # Test weak password
        weak_password = client.post('/register', data={
            'username': 'testuser',
            'email': 'test@example.com',
            'password': 'weak',
        ...
        duplicate_username = client.post('/register', data={
    ...
    def test_login_form_validation(self, client, user_manager):
        ...
        missing_password = client.post('/login', data={
            'username': 'testuser'
        }, follow_redirects=True)
        assert missing_password.status_code == 200

        # Test invalid credentials
        invalid_creds = client.post('/login', data={
            'username': 'testuser',
        ...
        nonexistent_user = client.post('/login', data={
    ...
    def test_credential_form_validation(self, client, user_manager):
        ...
        missing_platform = client.post('/credentials/add', data={
            'credential_type': 'api',
            'username': 'user'
        }, follow_redirects=True)
        ...
        invalid_platform = client.post('/credentials/add', data={
...
class TestErrorHandling:
    ...
    def test_403_forbidden_for_non_admin(self, client, user_manager):
        ...
        assert tasks_list.status_code == 200  # Tasks viewing is open to all authenticated users
