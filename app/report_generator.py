...
class ReportGenerator:
    ...
    def _get_tweets_in_period(self, user_id: int, days: int) -> List[Dict[str, Any]]:
        ...
        for row in rows:
            tweets.append({
                ...
                'total_engagement': (row['likes_count'] or 0) + (row['retweets_count'] or 0) + (row['replies_count'] or 0)
            })

        return tweets

    def _calculate_engagement_rate(self, likes: int, retweets: int, replies: int, impressions: int = None) -> float:
        """
    ...
    def generate_engagement_report(self, user_id: int, period: str, format: str) -> bytes:
        """
    ...
    def generate_growth_report(self, user_id: int, format: str) -> bytes:
        ...
        previous_tweets = [t for t in all_tweets if cutoff_previous <= t['created_at'] < cutoff_recent]
    ...
    def generate_top_tweets_report(self, user_id: int, limit: int, format: str) -> bytes:
        """
    ...
    def export_data(self, user_id: int, data_type: str, format: str) -> bytes:
        ...
        if data_type not in supported_types:
            raise ValueError(f'Unsupported data type: {data_type}. Supported: {", ".join(supported_types)}')
    ...
    def _format_csv_engagement(self, data: Dict[str, Any]) -> bytes:
        ...
        writer = csv.DictWriter(output, fieldnames=[
            'period', 'period_days', 'total_tweets', 'total_likes',
            'total_retweets', 'total_replies', 'total_engagement',
            'avg_engagement_rate', 'generated_at'
        ])
    ...
    def _format_csv_growth(self, data: Dict[str, Any]) -> bytes:
        ...
        output = io.StringIO()
        writer = csv.DictWriter(output, fieldnames=[
            'period_days', 'first_period_tweets', 'second_period_tweets',
            'tweets_change', 'engagement_trend', 'generated_at'
        ])
    ...
    def _format_csv_top_tweets(self, tweets: List[Dict[str, Any]]) -> bytes:
        ...
        writer = csv.DictWriter(output, fieldnames=[
            ...
            'total_engagement', 'has_media', 'is_thread'
        ])
    ...
    def _format_csv_export(self, data: List[Dict[str, Any]], data_type: str) -> bytes:
        ...
        if data_type == 'tweets' or data_type == 'engagement':
            writer = csv.DictWriter(output, fieldnames=[
                'tweet_id', 'text', 'likes', 'retweets', 'replies',
                'total_engagement', 'created_at', 'has_media', 'is_thread'
            ])
            ...
            for item in data:
                writer.writerow({
                    ...
                    'created_at': datetime.fromtimestamp(item['created_at']).isoformat() if item.get('created_at') else '',
...
"""
    ...
    def _format_html_top_tweets(self, tweets: List[Dict[str, Any]], limit: int) -> bytes:
