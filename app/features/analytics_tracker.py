...
class AnalyticsTracker:
    ...
    def get_top_tweets(
        self, user_id: int, metric: str = "engagement_rate", limit: int = 10
    ) -> List[dict]:
        """
        ...
        try:
            ...
            query = f"""
                ...
                LIMIT ?
            """  # nosec B608 - order_by_column validated via whitelist dict
    ...
    def get_snapshots(self, user_id: int, limit: Optional[int] = None) -> List[dict]:
        """
        Get analytics snapshots for a user.
        ...
        Returns:
            List of snapshot dictionaries with id, user_id, period, data, created_at
        """
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            ...
            # Build query
            query = """
                SELECT id, user_id, period, period_start, total_tweets,
                       total_impressions, total_engagements, avg_engagement_rate,
                       top_tweet_id
            ...
            results = []
            for row in rows:
                # Build data dictionary from snapshot fields
                data = {
                    "total_tweets": row[4],
                ...
                results.append(
                    {
                        ...
                        "period": row[2],
                        "data": data,
                        "created_at": row[3],  # period_start serves as created_at
                    }
                )
